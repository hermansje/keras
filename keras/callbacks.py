...
class CallbackList(object):
    ...
    def on_fit_batch_begin(self, batch, logs=None):
        ...
        for callback in self.callbacks:
            ...
            callback.on_fit_batch_end(batch, logs)
        self._delta_ts_fit_batch_begin.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_fit_batch_begin)
        if (self._delta_t_batch > 0. and
           delta_t_median > 0.95 * self._delta_t_batch and
           delta_t_median > 0.1):
            warnings.warn('Method on_train_batch_begin() is slow compared '
    ...
    def on_fit_batch_end(self, batch, logs=None):
        ...
        for callback in self.callbacks:
            ...
            callback.on_fit_batch_begin(batch, logs)
        self._delta_ts_fit_batch_end.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_fit_batch_end)
        if (self._delta_t_batch > 0. and
           (delta_t_median > 0.95 * self._delta_t_batch and delta_t_median > 0.1)):
            warnings.warn('Method on_train_batch_end() is slow compared '
                          'to the batch update (%f). Check your callbacks.'
                          % delta_t_median)
    ...
    def on_evaluate_batch_begin(self, batch, logs=None):
        ...
        t_before_callbacks = time.time()
        for callback in self.callbacks:
            callback.on_evaluate_batch_begin(batch, logs)
        self._delta_ts_evaluate_batch_begin.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_evaluate_batch_begin)
        if (self._delta_t_batch > 0. and
           delta_t_median > 0.95 * self._delta_t_batch and
           delta_t_median > 0.1):
    ...
    def on_evaluate_batch_end(self, batch, logs=None):
        ...
        t_before_callbacks = time.time()
        for callback in self.callbacks:
            callback.on_evaluate_batch_end(batch, logs)
        self._delta_ts_evaluate_batch_end.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_evaluate_batch_end)
        if (self._delta_t_batch > 0. and
           (delta_t_median > 0.95 * self._delta_t_batch and delta_t_median > 0.1)):
    ...
    def on_predict_batch_begin(self, batch, logs=None):
        ...
        self._delta_ts_predict_batch_begin.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_predict_batch_begin)
        if (self._delta_t_batch > 0. and
           delta_t_median > 0.95 * self._delta_t_batch and
           delta_t_median > 0.1):
            warnings.warn('Method on_batch_begin() is slow compared '
    ...
    def on_predict_batch_end(self, batch, logs=None):
        ...
        self._delta_ts_predict_batch_end.append(time.time() - t_before_callbacks)
        delta_t_median = np.median(self._delta_ts_predict_batch_end)
        if (self._delta_t_batch > 0. and
           (delta_t_median > 0.95 * self._delta_t_batch and delta_t_median > 0.1)):
            warnings.warn('Method on_batch_end() is slow compared '
                          'to the batch update (%f). Check your callbacks.'
